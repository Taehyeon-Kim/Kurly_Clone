...
class ReviewDataManager {
    func getReviews(productID: String, delegate: ProductReviewsViewController) {
        let url = "\(Constant.BASE_URL)/app/products/\(productID)/reviews"
            ...
            .responseDecodable(of: ReviewResponse.self) { response in
                ...
                case .success(let response):
                    ...
                    if response.isSuccess, let result = response.result {
                        delegate.getReviews(result: result[0].reviewList)
                    }
                    // 실패했을 때
                    else {
