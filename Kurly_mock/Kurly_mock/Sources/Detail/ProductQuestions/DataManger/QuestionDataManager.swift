...
class QuestionDataManager {
    func getQuestions(productID: String, delegate: ProductQuestionsViewController) {
        let url = "\(Constant.BASE_URL)/app/products/\(productID)/questions"
            ...
            .responseDecodable(of: QuestionResponse.self) { response in
                ...
                case .success(let response):
                    ...
                    if response.isSuccess, let result = response.result {
                        delegate.getQuestions(result: result)
                    }
                    // 실패했을 때
                    else {
