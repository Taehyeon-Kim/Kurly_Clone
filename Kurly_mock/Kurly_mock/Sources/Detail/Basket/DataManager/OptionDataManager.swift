...
class OptionDataManager {
    func getOptions(productID: String, delegate: BasketViewController) {
        let url = "\(Constant.BASE_URL)/app/products/\(productID)/options"
            ...
            .responseDecodable(of: OptionResponse.self) { response in
                ...
                case .success(let response):
                    ...
                    if response.isSuccess, let result = response.result {
                        delegate.getOptions(result: result)
                    }
                    // 실패했을 때
                    else {
